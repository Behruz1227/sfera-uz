...
const Module = () => {
  ...
  const {
    ...
    `${post_question}${currentLessonId}?nextLessonId=${nextLessonId ? nextLessonId : 0}`,
    result,
    Config()
  );
  const { data: lessonData, getData: getLesson } = useGet(
  ...
  const modules = data.length > 0 && data?.map((item: moduledata) => ({
    moduleId: item?.moduleId,
    name: item?.name,
    lessonCount: item?.lessonCount,
  }));

  const lessons = lessonData.length > 0 && lessonData?.map((item: lessondata) => ({
    moduleId: item?.moduleId,
    name: item?.name,
    lessonId: item?.id,
  ...
  return (
    <SidebarDemo>
      ...
      <div
        className={`relative ${data && data.length > 0 ? "p-2 md:p-10 overflow-y-auto" : "p-2 md:p-10 overflow-hidden"} w-full min-h-screen  dark:bg-black bg-[${bgColorBody}] dark:bg-dot-white/[0.2] bg-dot-black/[0.3]`}
      >
        ...
        <div className={` w-full ${data && data.length > 0 && "lg:pe-[305px]"} p-2`}>
          <Breadcrumbs text="Lesson" className="" />
        </div>
        {data && data.length > 0 ? (
          <div className="flex flex-col lg:flex-row w-full justify-between">
            <div className="flex flex-col w-full lg:mr-[277px] p-2">
              {VedioLink ? (
                <VideoPlayer videoId={VedioLink} />
              ) : (
                <>
                  ...
                  <div>
                    {questionData.map((question: questiondata) => (
                      <div key={question?.id} className="mb-6">
                        ...
                        <RadioGroup
                          value={
                            selectedAnswers[question?.id ? question?.id : 0]
                          }
          ...
          >
            ...
            <h1
              className={` text-xl bg-transparent font-bold text-center text-[#000] relative z-20`}
            >
              Modul topilmadi☹
            </h1>
          </div>
